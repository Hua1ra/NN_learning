...
After that we have a (400, 5) vector of predictions. Adding .view(20, 20, 5) we'll achieve the data at the same format as the target labels. That data can be interpreted properly in the visualization part.



##### 4\) Results

After 40 hours of training process (device: GPU AMD RX 6800S) we've reached a point where the model predicts most of the obvious signs. For the validation a completely different dataset was used (CCTSDB). That is the main reason for metrics to drop. I also need to specify that Non-Maximum suppression was used to get more accurate results
